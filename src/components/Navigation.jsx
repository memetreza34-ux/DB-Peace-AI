  ...
  return (
    <header className="sticky top-0 z-40 border-b border-db-dark/10 bg-white/95 shadow-sm backdrop-blur-md dark:border-white/10 dark:bg-db-dark/95">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-3 px-4 py-3 sm:px-6">
        <button type="button" onClick={() => setActiveTab("home")} className="group flex min-w-0 items-center gap-3 rounded-xl text-left outline-none focus:ring-2 focus:ring-db-red/30" aria-label="DB Peace Startseite öffnen">
          <div className="flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-lg bg-db-red text-sm font-black text-white shadow-sm transition group-hover:scale-105">DP</div>
        ...
        <nav className="flex shrink-0 items-center gap-1.5 sm:gap-2" aria-label="Hauptnavigation">
          ...
          <button type="button" onClick={onOpenEmergency} className="flex h-10 items-center justify-center gap-2 rounded-xl bg-red-600 px-3 text-xs font-black text-white shadow-sm transition hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500/40 sm:px-4 sm:text-sm" aria-label="Akute Hilfe öffnen">
...
function NavButton({ active, hideLabelOnMobile = false, icon: Icon, label, onClick }) {
  return (
    <button type="button" onClick={onClick} aria-label={label} className={`flex h-10 items-center justify-center gap-2 rounded-xl px-3 text-xs font-black transition focus:outline-none focus:ring-2 focus:ring-db-red/30 sm:px-4 sm:text-sm ${active ? "bg-db-dark text-white shadow-sm dark:bg-white dark:text-db-dark" : "text-db-dark/70 hover:bg-db-dark/5 hover:text-db-dark dark:text-white/70 dark:hover:bg-white/10 dark:hover:text-white"}`} aria-current={active ? "page" : undefined}>
