        ...
        <div className="text-center space-y-6 py-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-8 text-left">
            ...
            >
              ...
              <p className="text-sm font-semibold text-ink-muted">Privates Gedächtnisprotokoll anlegen, um Fakten sofort zu sichern.</p>
            </button>
            ...
            >
              ...
              <p className="text-sm font-semibold text-ink-muted">Einen Vorfall offiziell, sachlich und auf Wunsch anonym melden.</p>
            </button>
            ...
            >
              ...
              <p className="text-sm font-semibold text-ink-muted">Schreibe oder diktiere frei, was passiert ist. Die KI füllt das Formular für dich aus.</p>
            </button>
          </div>
        </div>
      )}
              ...
              <div className="flex items-center gap-2">
                {records.length > 0 && (
                  <button
                    ...
                    className="flex min-h-11 items-center gap-1.5 rounded-xl border border-line/15 px-3 py-2 text-xs font-bold text-ink-muted hover:border-db-red hover:text-db-red transition"
                  >
            ...
            {showForm && (
              <form onSubmit={handleAddRecord} className="mt-4 p-4 rounded-xl bg-db-warm/50 dark:bg-db-dark/30 border border-line/10 space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div>
                    <label className="text-xs font-bold text-ink-muted">Datum</label>
                    <input
                      type="date"
                      value={newDate}
                      onChange={(e) => setNewDate(e.target.value)}
                      className="mt-1 w-full rounded-lg border border-line/15 bg-surface dark:text-white p-2 text-xs font-semibold"
                    />
                  </div>
                  <div>
                    <label className="text-xs font-bold text-ink-muted">Uhrzeit</label>
                    <input
                      type="time"
                      value={newTime}
                  ...
                  <div>
                    <label className="text-xs font-bold text-ink-muted">Ort / Kontext</label>
                    <input
                      type="text"
                ...
                <div>
                  <label className="text-xs font-bold text-ink-muted">Was genau ist passiert? (Ohne Klarnamen)</label>
                  <textarea
                    rows={3}
                    placeholder="Beschreibe den Vorfall sachlich: Wer hat was gesagt/getan?"
                    value={newDesc}
                    onChange={(e) => setNewDesc(e.target.value)}
                    className="mt-1 w-full rounded-lg border border-line/15 bg-surface dark:text-white p-2.5 text-xs font-semibold"
                  />
                </div>
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="flex items-center gap-1.5 rounded-lg border border-line/15 bg-surface px-3 py-1.5 text-xs font-bold text-ink hover:bg-db-warm dark:hover:bg-white/5 transition"
                    >
                    ...
                    <div className="flex flex-wrap gap-2 mt-1">
                      {newFiles.map((f, idx) => (
                        <div key={idx} className="flex items-center gap-1.5 rounded-md bg-surface border border-line/10 px-2 py-1 shadow-sm">
                          <span className="text-[10px] font-semibold text-ink truncate max-w-[120px]">{f.name}</span>
                ...
                <div className="flex justify-end gap-2">
                  <button
                    ...
                    className="rounded-lg px-3 py-1.5 text-xs font-bold text-ink-muted hover:bg-db-dark/5 dark:hover:bg-white/5"
                  >
                    Abbrechen
                  </button>
                  <button
                  ...
                  >
                    <div className="flex items-center justify-between text-xs font-bold text-ink-muted">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="flex items-center gap-1 font-bold text-ink-muted">
                      ...
                      <div className="mt-3 flex flex-wrap gap-2 pt-2 border-t border-db-dark/5 dark:border-white/5">
                        {r.files.map((file, idx) => (
                          <div key={idx} className="flex items-center gap-1.5 rounded-lg bg-db-warm/50 dark:bg-db-dark/50 border border-line/10 px-2 py-1">
                            ...
                            <span className="text-[11px] font-semibold text-ink">{file.name}</span>
                          </div>
                        ))}
                      </div>
                    )}
      ...
      {selectedRecord && (
        ...
        >
          ...
          >
            ...
            <div className="flex flex-wrap items-center gap-3 mb-6 text-sm font-bold text-ink-muted">
              <span className="flex items-center gap-1 text-ink">
                  ...
                  {selectedRecord.files.map((file, idx) => (
                    <div key={idx} className="rounded-xl border border-line/10 bg-db-warm/10 dark:bg-db-dark/30 overflow-hidden group">
                      ...
                      <div className="bg-surface p-2">
                        <p className="truncate text-[10px] font-bold text-ink">{file.name}</p>
                        {!file.gespeichert && (
            ...
            <div className="mt-8 flex flex-col-reverse gap-3 sm:flex-row sm:justify-between">
              <button
                ...
                className="inline-flex min-h-11 items-center justify-center gap-2 rounded-lg border border-line/15 px-5 py-2.5 text-sm font-bold text-ink-muted transition hover:border-db-red hover:text-db-red"
              >
                <Trash2 className="h-4 w-4" />
                Eintrag löschen
              </button>
