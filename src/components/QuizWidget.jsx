...
export function QuizWidget() {
  ...
  async function loadQuestions(signal) {
    setIsLoading(true);
    setNotice("");
    ...
    try {
      ...
      startQuiz(validated, "ki");
    } catch (error) {
      if (error.name === "AbortError") return;
      startQuiz(fallbackQuestions, "local");
      setNotice("Der KI-Dienst ist nicht verfügbar. Deshalb werden geprüfte lokale Übungsfragen verwendet.");
    } finally {
      setIsLoading(false);
    }
  }
  ...
  if (isLoading) {
    return (
      <div className="flex h-64 w-full flex-col items-center justify-center gap-4 rounded-xl border border-db-dark/10 bg-white shadow-md dark:border-white/10 dark:bg-db-dark/80">
  ...
  if (questions.length === 0) {
    return (
      <div className="rounded-xl border border-amber-200 bg-amber-50 p-5 text-sm font-semibold text-amber-950">
      ...
      <div className="relative w-full overflow-hidden rounded-xl bg-db-dark p-8 text-center text-white shadow-md">
        ...
        <motion.div initial={{ scale: 0.92, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}>
          ...
          <button
            ...
            className="mx-auto mt-6 inline-flex items-center justify-center gap-2 rounded-xl bg-white/10 px-5 py-3 text-sm font-black transition hover:bg-white/20"
          >
  ...
  return (
    <div className="relative w-full overflow-hidden rounded-xl border border-db-dark/10 bg-white p-6 shadow-md dark:border-white/10 dark:bg-db-dark/80">
      <div className="mb-5 flex flex-wrap items-center justify-between gap-2">
        <p className="text-xs font-black uppercase tracking-wider text-db-red">
          Wissens-Quiz · Frage {currentQIndex + 1} von {questions.length}
        </p>
        <span className="rounded-full bg-db-soft px-3 py-1 text-[10px] font-black text-db-rail dark:bg-white/10 dark:text-white/60">
          {source === "ki" ? "KI-Fragen · ungeprüfte Lernhilfe" : "Lokale Übungsfragen"}
        </span>
      </div>

      {notice && (
        <div className="mb-5 flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs font-semibold leading-5 text-amber-950">
      ...
      <div className="mb-6 flex items-center gap-1" aria-hidden="true">
        {questions.map((question, index) => (
          <div
            key={question.id}
            className={`h-1.5 flex-1 rounded-full ${index <= currentQIndex ? "bg-db-red" : "bg-db-dark/10 dark:bg-white/10"}`}
          />
        ))}
      </div>

      <h3 className="min-h-16 text-lg font-black leading-snug text-db-dark dark:text-white">
        ...
        {!showResult ? (
          ...
          >
            <div className="flex items-start gap-3">
              ...
              <div>
                <h4 className={`font-black ${isCorrect ? "text-emerald-800 dark:text-emerald-300" : "text-red-800 dark:text-red-300"}`}>
                  {isCorrect ? "Richtig" : "Nicht richtig"}
                </h4>
                <p className="mt-1 text-sm font-medium leading-6 text-db-dark/80 dark:text-white/75">
            ...
            <button
              ...
              className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl border border-db-dark/10 bg-white py-2.5 text-sm font-black text-db-dark transition hover:bg-db-soft dark:border-white/10 dark:bg-white/10 dark:text-white dark:hover:bg-white/15"
            >
...
function AnswerButton({ label, onClick }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="flex-1 rounded-xl border border-db-dark/10 bg-db-soft p-4 text-center font-black text-db-dark transition hover:border-db-red hover:bg-red-50 dark:border-white/10 dark:bg-white/5 dark:text-white dark:hover:border-db-red dark:hover:bg-db-red/10"
    >
      {label}
    </button>
  );
...
function validateQuestions(input) {
  ...
  return input
    .slice(0, 6)
    .map((question, index) => ({
      id: String(question?.id ?? `ki-${index + 1}`),
