...
const items = [
  ...
  },
  {
    ...
    description: "Fiktive Annahmen transparent ändern und rechnerische Ergebnisse vergleichen.",
    icon: BarChart3,
...
export function DashboardHome({ onNavigate, onOpenEmergency }) {
  ...
  const activeItemVariants = reduceMotion ? reducedVariants : itemVariants;

  return (
    <motion.div variants={activeContainerVariants} initial="hidden" animate="show" className="space-y-9">
      <motion.header variants={activeItemVariants} className="relative overflow-hidden rounded-2xl bg-db-dark px-6 py-10 text-center text-white shadow-xl sm:px-10 sm:py-14">
        ...
        {!reduceMotion && (
          <motion.div
            ...
            aria-hidden="true"
          >
            <TrainFront className="h-24 w-24" aria-hidden="true" />
            <div className="ml-2 h-14 w-28 rounded-xl border-2 border-current" />
            <div className="ml-2 h-14 w-28 rounded-xl border-2 border-current" />
          </motion.div>
        )}

        <Sparkles className="pointer-events-none absolute left-8 top-8 h-6 w-6 text-db-red" aria-hidden="true" />
        <Sparkles className="pointer-events-none absolute bottom-8 right-8 h-7 w-7 text-db-red" aria-hidden="true" />
        ...
        <div className="relative z-10 mx-auto max-w-3xl">
          <div className="mx-auto inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-1.5 text-xs font-black text-white/75">
            <FlaskConical className="h-3.5 w-3.5 text-violet-300" aria-hidden="true" />
      ...
      <motion.div variants={activeItemVariants} className="mx-auto max-w-lg">
        <MoodTracker />
      </motion.div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {items.map(({ id, title, description, icon: Icon, accent }) => (
          <motion.button
            ...
            className="group flex min-h-56 flex-col items-start rounded-xl border border-db-dark/10 bg-white p-6 text-left shadow-sm transition hover:border-db-red/35 hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-db-red/30 dark:border-white/10 dark:bg-db-dark/50"
          >
            ...
            <span className="mt-5 inline-flex items-center gap-2 text-xs font-black uppercase tracking-wide text-db-red">
              Öffnen <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" aria-hidden="true" />
            </span>
          </motion.button>
        ))}
      </div>
      ...
      <motion.div variants={activeItemVariants} className="grid gap-4 sm:grid-cols-2">
        ...
        <button type="button" onClick={() => onNavigate("privacy")} className="flex items-center gap-4 rounded-xl border border-emerald-200 bg-emerald-50 p-5 text-left transition hover:border-emerald-400 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-emerald-500/30 dark:border-emerald-900/50 dark:bg-emerald-950/25">
          <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-emerald-100 text-emerald-700 dark:bg-emerald-500/15 dark:text-emerald-300"><ShieldCheck className="h-6 w-6" aria-hidden="true" /></div>
          <div><h2 className="font-black text-emerald-950 dark:text-emerald-200">Datenschutzstatus</h2><p className="mt-1 text-xs font-semibold text-emerald-800/75 dark:text-emerald-300/70">Offene Grenzen, erforderliche Prüfungen und Voraussetzungen für einen Pilotbetrieb.</p></div>
        </button>
      </motion.div>
    </motion.div>
  );
